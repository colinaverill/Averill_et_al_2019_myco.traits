...
#setup to save.----
png(filename=out.path,width=17.8,height=11.5,units='cm',res=300)
...
plot(tr.form, data = d[d$Ngreen > -1,], cex = p.cex, pch = 16, col = alpha(d$col, trans), xlab = NA, ylab = NA, cex.axis = axis.cex, bty ='l')
...
mtext(label, side = 3, adj = 0.05, line = -1, cex = stat.cex)
mtext(units, side = 3, adj = 0.05, cex = unit.cex, line = -2.5)
mtext(bquote(R^2 == .(round(s.mod$r.squared, 2))), side = 3, adj = 0.05, line = -3.5, cex = unit.cex)
mtext(lambda.lab, side = 3, adj = 0.05, line = -4.5, cex = unit.cex)
...
s.mod <- summary(mod)
plot(d$Nsenes ~ d$Nsenes_estimate, cex = p.cex, pch = 16, col = alpha(d$col, trans), xlab = NA, ylab = NA, cex.axis = axis.cex, bty = 'l')
line.col <- ifelse(s.mod$coefficients[2,4] < 0.05,r.line.col,'gray')
line.type <- ifelse(s.mod$coefficients[2,4] < 0.05, 1,2)
abline(mod, lwd = 2, col = line.col, lty = line.type)
lambda.val <- z[z$analysis == trait,c('lambda')]
...
mod<- lm(tr.form, data = d)
s.mod <- summary(mod)
plot(tr.form, data = d, cex = p.cex, pch = 16, col = alpha(d$col, trans), xlab = NA, ylab = NA, cex.axis = axis.cex, bty = 'l')
line.col <- ifelse(s.mod$coefficients[2,4] < 0.05,r.line.col,'gray')
line.type <- ifelse(s.mod$coefficients[2,4] < 0.05, 1,2)
abline(mod, lwd = 2, col = line.col, lty = line.type)
lambda.val <- z[z$analysis == trait,c('lambda')]
...
#outer labels.----
mtext('observed trait', side = 2, outer = T, cex = 1.3, line = 1)
mtext('phylogenetic estimated trait', side = 1, outer = T, cex = 1.3, line = 1.65)
#legend.----
par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0), new = TRUE)
plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
legend('topright',legend = c('arbuscular mycorrhizal','ectomycorrhizal'), 
       pch = 16, col = cols, bty = 'n', cex = 1.3,
       x.intersp = .75, xpd = T, 
       horiz = T)
