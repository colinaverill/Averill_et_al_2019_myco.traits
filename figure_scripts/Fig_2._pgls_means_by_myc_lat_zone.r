...
#setup to save.----
png(filename=output.path,width=17.8,height=11.25,units='cm',res=300)
...
for(i in 1:6){
  ...
  arrows(x, lwr, x, upr, length=0.04, angle=90, code=3)
  #points(y ~ x, pch = 16, col = myc.col, cex = 2)
  points(y ~ x, pch = 21, col = 'black', bg = myc.col, cex = 1.5)
  
  #latitudinal zone text labels, sample size, significane labels.
  text(lat.lab, x = lat.lab.pos, y = limy[2] * 0.14, cex = stat.cex)
  text(  p.lab, x = lat.lab.pos, y = limy[2] * 0.09, cex = stat.cex)
  text(  N.lab, x = lat.lab.pos, y = limy[2] * 0.03, cex = stat.cex)
  #trait label.
  mtext(trait.name, side = 3, adj = 0.02, line = 0.3, cex = trait.lab.cex)
  
  #unit labels on axes of plots 1 and 4.----
  if(i == 1){mtext(expression(paste('mg N (g tissue)'^'-1')), side = 2, line = 2.5, cex = trait.lab.cex)}
  if(i == 4){mtext(expression(paste('mg P (g tissue)'^'-1')), side = 2, line = 2.5, cex = trait.lab.cex)}
}
#Drop legend.----
par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0), new = TRUE)
plot(0, 0, type = "n", bty = "n", xaxt = "n", yaxt = "n")
legend('bottom',legend = c('arbuscular mycorrhizal','ectomycorrhizal'), 
       pch = 16, col = myc.col, bty = 'n', cex = 1.3,
       x.intersp = .75, xpd = T, 
       horiz = T)
